#!/usr/bin/env python3
"""Guarded piper demo mirror recorder for Piper LeRobot datasets."""
...
from piper_driver.piper_bus import PiperMotorsBusV2, PiperMotorsBusV2Config
from piper_driver.reset import reset_to_standard_start
from piper_driver.schema import (
    DUAL_CAMERA_KEYS,
    GLOBAL_CAMERA_KEY,
    QposTolerance,
    STANDARD_START_QPOS,
...
from piper_driver.start_pose import qpos_diff, start_pose_guard
from camera.rs_camera import RealSenseCamera, USBCamera
from teleop import data_collector as collector

DEFAULT_DATASET_ROOT = PROJECT_ROOT / "data" / "lerobot_dataset_piper_demo"
DEFAULT_DATASET_REPO_ID = "piper/demo"
DEFAULT_TASK = "Piper hardware mirror demonstration"
MIN_SAVE_FRAMES = 1
...
def create_global_camera(device_id: str):
    print("\nInitializing piper demo global camera ...")
...
def create_or_resume_dataset(args):
    ...
    _include_wrist = getattr(args, "include_wrist", False)
    dataset = collector.create_or_resume_dataset(
        dataset_cls,
        Path(args.dataset_root),
        args.dataset_repo_id,
        include_wrist=_include_wrist,
    )
...
def save_episode_start_metadata(
    ...
    *,
    ...
    metadata_dir = dataset_root / "meta" / "episode_metadata"
...
def draw_status_preview(cv2, preview, *, state: RecordState, episode_id: int, dry_run: bool, guard_passed: bool = False):
    ...
    lines = [
        f"piper demo {'DRY RUN' if dry_run else 'RECORD'}",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Safe piper demo Piper recorder with a required per-episode start guard."
    )
    ...
    parser.add_argument(
        "--expected-start-qpos",
        default="",
        help="Comma-separated [j1,j2,j3,j4,j5,j6,gripper]. Default uses piper demo schema.",
    )
    parser.add_argument(
        "--start-guard-mode",
        choices=("strict", "zone"),
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Always enabled. Recording cannot bypass the piper demo start guard.",
    )
    ...
    _valid_keys = {GLOBAL_CAMERA_KEY, WRIST_CAMERA_KEY}
    if args.camera_key not in _valid_keys:
        parser.error(f"--camera-key must be one of {_valid_keys}.")
    args.include_wrist = not args.no_wrist
    if args.camera_key == WRIST_CAMERA_KEY and args.no_wrist:
        parser.error(f"--camera-key={WRIST_CAMERA_KEY} is incompatible with --no-wrist.")
...
def main() -> int:
    ...
    print("=" * 72)
    print("Piper Demo Piper guarded recorder")
    ...
    if args.include_wrist:
        print(f"  camera topology   : dual — global USB + wrist RealSense")
        print(f"  wrist serial      : {'auto' if not args.wrist_serial else args.wrist_serial}")
    else:
    ...
    wrist_cam = None
    dataset = None
    window_name = "Piper Demo Recorder | Global"
    ...
    try:
        ...
        if not args.dry_run:
            ...
            if args.include_wrist:
                print("\n[3/3] Wrist RealSense camera ...")
                try:
                    wrist_cam = RealSenseCamera(
                        serial=args.wrist_serial or None,
                        width=collector.GLOBAL_WIDTH,
                        height=collector.GLOBAL_HEIGHT,
                        fps=collector.GLOBAL_FPS,
                    )
                    print(f"  Wrist RealSense ready: {wrist_cam.width}x{wrist_cam.height} @ {wrist_cam.fps}fps")
                except Exception as exc:
                    print(f"  [WARN] Wrist camera init failed: {exc}")
                    print("  Continuing with global camera only.")
                    wrist_cam = None
            else:
                print("\n[3/3] Wrist camera disabled (--no-wrist).")
        else:
        ...
        while True:
            ...
            if wrist_cam is not None:
                wrist_frame = collector.safe_read_camera("wrist", wrist_cam, camera_error_log)
            ...
            if state in (RecordState.RECORDING, RecordState.WAIT_FOR_USER_STOP):
                ...
                if not args.dry_run and dataset is not None and cur_qpos is not None:
                    _camera_ok = global_frame is not None
                    if args.include_wrist and wrist_frame is None:
                        _camera_ok = False
                    if prev_qpos is not None and _camera_ok:
                        frame = {
                            "observation.state": np.asarray(prev_qpos, dtype=np.float32),
                            "action": np.asarray(cur_qpos, dtype=np.float32),
                            "task": args.task,
                            GLOBAL_CAMERA_KEY: np.transpose(global_frame.rgb, (2, 0, 1)),
                        }
                        if args.include_wrist:
                            frame[WRIST_CAMERA_KEY] = np.transpose(wrist_frame.rgb, (2, 0, 1))
                        try:
                            dataset.add_frame(frame)
                        except Exception as exc:
                            print(f"  [WARN] dataset.add_frame failed: {exc}")
                    elif prev_qpos is not None and not _camera_ok:
                        now = time.time()
                        _missing = []
                        if global_frame is None:
                            _missing.append("global")
                        if args.include_wrist and wrist_frame is None:
                            _missing.append("wrist")
                        if now - camera_error_log.get("frame_skip", 0.0) > 2.0:
                            print(f"  [WARN] recording frame skipped: {', '.join(_missing)} frame unavailable")
            ...
            preview = collector.build_preview(
                wrist_frame if args.include_wrist else None,
                global_frame,
                bus.is_enabled,
                state == RecordState.WAIT_FOR_USER_STOP,
                frame_count,
            ...
            if key in (ord("c"), ord("C")):
                ...
                if state == RecordState.WAIT_FOR_USER_STOP:
                    ...
                    if not guard_result.passed:
                        ...
                        state = RecordState.WAIT_FOR_START_GUARD
                        request_guard_check = False
                    elif not args.dry_run and (
                        is_frame_black(global_frame)
                        or (args.include_wrist and is_frame_black(wrist_frame))
                    ):
                        _black = []
                        if is_frame_black(global_frame):
                            _black.append("global")
                        if args.include_wrist and is_frame_black(wrist_frame):
                            _black.append("wrist")
                        print(f"  [WARN] Camera frame(s) black: {', '.join(_black)}. Check camera connection.")
                        print(f"  Selected global device: {global_cam.device_id if global_cam else 'none'}")
                        guard_passed = True
                    else:
                        guard_passed = True
        ...
        print("  Arm stays ENABLED when piper demo recorder exits.")
        bus.disconnect()
        if global_cam is not None:
            global_cam.close()
        if wrist_cam is not None:
