#!/bin/bash
# ============================================================
# ACT Training — Old Single-Camera 10-Demo Piper Bottle Baseline
# ============================================================
# Dataset:
...
#
# Camera:
#   Use exactly the dataset camera key:
...
DATASET_ROOT="${DATASET_ROOT:-${PROJECT_ROOT}/data/lerobot_dataset_piper_bottle_old_singlecam_10demo}"
DATASET_REPO_ID="${DATASET_REPO_ID:-piper/old_singlecam_10demo}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/outputs/train/act_old_singlecam_10demo}"
STEPS="${STEPS:-3000}"
...
echo "================================================"
echo "  ACT Old Single-Camera 10-Demo Training"
echo "================================================"
...
if [ ! -f "${DATASET_ROOT}/meta/info.json" ] || [ ! -d "${DATASET_ROOT}/data" ]; then
    ...
    echo "        Run scripts/select_old_singlecam_10demo.py first." >&2
    exit 1
fi
...
if [ "${DEVICE}" = "cpu" ] && [ "${ALLOW_CPU_TRAINING}" != "1" ]; then
    echo "[ERROR] Refusing CPU training by default; this baseline is intended to be fast." >&2
...
PYTHONPATH= "${PYTHON_BIN}" -m lerobot.scripts.lerobot_train \
    ...
    --job_name=act_old_singlecam_10demo
