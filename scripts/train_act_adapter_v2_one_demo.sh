#!/bin/bash
# ============================================================
# ACT Training — Adapter v2 One-Demo Piper Bottle
# ============================================================
# Dataset:
#   data/lerobot_dataset_piper_adapter_v2/
#
# Output:
#   outputs/train/act_adapter_v2_one_demo/
#
# Camera:
#   observation.images.global_rgb  (single camera, adapter v2)
#
# This is the first adapter v2 training run. Only proceed after
# the full Stage 1 chain has passed:
#   smoke → gripper → start_pose → record → dataset sanity → replay
# ============================================================
set -euo pipefail
...
DATASET_ROOT="${DATASET_ROOT:-${PROJECT_ROOT}/data/lerobot_dataset_piper_adapter_v2}"
DATASET_REPO_ID="${DATASET_REPO_ID:-piper/adapter_v2}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/outputs/train/act_adapter_v2_one_demo}"
STEPS="${STEPS:-1000}"
...
echo "================================================"
echo "  ACT Adapter v2 One-Demo Training"
echo "================================================"
...
if [ ! -f "${DATASET_ROOT}/meta/info.json" ] || [ ! -d "${DATASET_ROOT}/data" ]; then
    ...
    echo "        Record one demo with scripts/record_adapter_v2_mirror.py first." >&2
    exit 1
fi
...
PYTHONPATH= "${PYTHON_BIN}" -m lerobot.scripts.lerobot_train \
    ...
    --job_name=act_adapter_v2_one_demo
