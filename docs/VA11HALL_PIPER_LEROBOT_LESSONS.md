...
units, CAN names, gripper scale, camera devices, start pose, motion limits, and
...
This topology also removes automatic reset from the active teaching flow. The
operator returns the teaching/follower pair to an approximate start pose
manually. Adapter-v2 code may read that pose and gate recording on a tolerance,
but it must not command a `q_start` reset.
...
我们不应该为了“更像标准 LeRobot”立刻推翻现在能工作的安全 adapter。更合理的路线是保留当前 Piper CAN、manual start guard、gripper scale、rollout logging 等安全层，然后逐步把代码组织成更清晰的 LeRobot-style robot abstraction。
...
- startup/start 检查不能省略。

对当前项目的含义：

- `open-gripper-on-start` 是必要的，不是临时补丁。
- 录制前人工摆回大致一致的 start pose 是必要的。
- manual start guard 是必要的。
...
这些工具帮助我们查清连接、夹爪、start pose、chunk execution、wrist freeze 等问题，但它们不应该变成未来所有模型和所有数据集的默认假设。
